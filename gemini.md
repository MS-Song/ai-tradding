...
### D. 랭킹 시스템 (Ranking System)
* **데이터 소스**: KIS API 대신 **네이버 금융** 실시간 인기 검색 및 거래량 상위 데이터 사용.
* **수집 및 노출**: 
    * 각 섹션별 최대 **100개** 종목 수집.
    * 화면(TUI)에는 상위 **10개**만 출력하여 가독성 유지.
* **필터링 (Safety Filter)**:
    * 상장폐지 및 급락 위험 방지를 위해 **관리종목, 거래정지, 투자환기, 정리매매, 단기과열** 종목은 수집 대상에서 원천 배제.
    * 우선주(`우`, `우A`, `우B` 등) 제외 로직 유지.
