...
def draw_performance_report(strategy, dm):
    ...
    while True:
        ...
        if current_tab == 1:
            ...
            # ③ [좌우 배치] 매수/매도 테이블
            other_w = 70 # 가격(11)|현재(11)|평균(11)|손익(13)|방법(11)|평가(7) = 64 + 6 separators
            half_w = tw // 2
            ...
            def format_trade_row(info, is_buy):
                ...
                row = (f"{smart_align(f'[{code}]{name}', name_w)}|"
                       f"{align_kr(f'{int(price):,}', 11, 'right')}|"
                       f"{align_kr(f'{int(cur):,}', 11, 'right')}|"
                       f"{align_kr(ma_str, 11, 'right')}|"
                       f"{p_color}{align_kr(f'{int(pnl):,}', 13, 'right')}\033[0m|"
                       f"{align_kr(info['type'][:5], 11)}|"
                       f"{v_color}{align_kr(verdict, 7)}\033[0m")
                return row
            ...
            t_head = f"{smart_align('종목(코드)명', name_w)}|{align_kr('매수가', 11)}|{align_kr('현재가', 11)}|{align_kr('평균선', 11)}|{align_kr('평가손익', 13)}|{align_kr('방법', 11)}|{align_kr('평가', 7)}"
            t_head_s = f"{smart_align('종목(코드)명', name_w)}|{align_kr('매도가', 11)}|{align_kr('현재가', 11)}|{align_kr('평균선', 11)}|{align_kr('실현손익', 13)}|{align_kr('방법', 11)}|{align_kr('평가', 7)}"
